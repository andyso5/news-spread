        ...
        for i in essay_list:
            ...
            if next_page and essay_list:
                ...
                if self.page_switch_test():
                    ...
                    return self.collect_info(title,next_page=False,read_count=next_page)
...
class Weibo(Web):#搜索
    ...
    def check_box(self,cover_num=1):
        ...
        first_img_obj = self._wait_for(5,(By.XPATH,'.//div[@class="list_wrap"]//ul/li[1]/*[@class="pic"]'))
        first_img_style = first_img_obj.get_attribute("style") # 包含url，相同图片胡url不相同
        ...
        time.sleep(3.5)#如不等待，封面容易变成前一个，而不是目前在上传的。
        for _ in range(6):
            current_first_img_obj = self._wait_for(5,(By.XPATH,'.//div[@class="list_wrap"]//ul/li[1]/*[@class="pic"]'))
            style_str = current_first_img_obj.get_attribute("style")
            if style_str != first_img_style:
                self._js_click(current_first_img_obj)
                break
...
class Sohu(Web):#翻页
    ...
    def check_box(self,cover_num=1,again=True):
        ...
        #封面
        self._load_cover(cover_set,again=again)
        # cover_set.click()
        # res = Winhand().upload_file(self.message['cover_path'])
        # if not res:
        #     print('信息来源：%s' %self.source)
        #     if again:
        #         time.sleep(1)
        #         self.check_box(again=False)
        #     else:
        #         print('第二次上传图片失败。\n')

        self._wait_for_displayed(5,(By.XPATH,'.//div[@class="crop-btn"]/button[text()="确定"]')).click()
        self._wait_for_displayed(10,(By.XPATH,'.//div[@class="cover-area"]/div[@class="cover-place"]'))
...
class Toutiao(Web):#搜索
    ...
    def check_box(self,cover_num=1):
        triple_img_path = './/span[text()="三图"]/../..'
        triple = self._wait_for(2,(By.XPATH, triple_img_path))
        ...
        if self.message['imgs_num'] > 2:
            ...
            single = self._wait_for(2,(By.XPATH,'.//span[text()="单图"]/../..'))
            # single.click()
            self._js_click(single)
            # select = self._wait_for(2,(By.XPATH,'.//div[@class="article-cover-images"]/div/i'))
            # self._js_click(select)
        ...
        for i in essay_list:
            ...
            if next_page:
                ...
                if self.page_switch_test():
                    ...
                    return self.collect_info(title,next_page=next_page,read_count=next_page,endless=endless)
...
class Tpy(Web):
    ...
    def _change_cover_size(self,target_size =  (900,600),img_format='jpg'):
        img  = CompressImage(self.message['cover_path'])
        if img.size[0] < target_size[0] or img.size[1] < target_size[1]:
...
class Cheshi(Web):
    ...
    def collect_info(self,title,next_page=False,read_count=False,endless=False,times=5):
        ...
        for i in essay_list:
            ...
            if next_page and times:
                ...
                if self.page_switch_test():
                    ...
                    return self.collect_info(title,next_page=next_page,read_count=next_page,endless=endless,times=times)
